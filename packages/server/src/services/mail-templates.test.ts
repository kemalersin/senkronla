...
describe('buildDeveloperMailTemplate', () => {
  it('renders branded HTML with CTA and fallback link', () => {
    const template = buildDeveloperMailTemplate({
      locale: 'en',
      kind: 'email_verify',
      link: 'https://sync.senkron.la/en/developer/verify?token=abc',
      brandName: 'Senkronla',
      webBaseUrl: 'https://sync.senkron.la',
    })

    expect(template.subject).toBe('Verify your developer account')
    expect(template.text).toContain('Verify your email')
    expect(template.text).toContain('https://sync.senkron.la/en/developer/verify?token=abc')
    ...
    expect(template.html).toContain('https://sync.senkron.la/en/developer/verify?token=abc')
  })
  ...
  it('localizes Turkish password reset copy', () => {
    const template = buildDeveloperMailTemplate({
      locale: 'tr',
      kind: 'password_reset',
      link: 'https://sync.senkron.la/tr/developer/reset-password?token=xyz',
      brandName: 'Senkronla',
      webBaseUrl: 'https://sync.senkron.la',
    })
