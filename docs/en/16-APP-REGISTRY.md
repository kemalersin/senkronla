...
## 20. Implementation plan (done)

### Phase A — Core registry (5–7 days)

- [x] Migration `006_app_registry.sql`
- [x] Config schema + seed loader
- [x] App context middleware (web Origin + native bundle)
- [x] Namespace `app_uuid` on create; cross-check on device auth
- [x] Dynamic CORS
- [x] Error codes + tests
- [x] SDK header injection + `appId` option

**Done when:** operator_managed web app can create namespace; wrong origin rejected.

### Phase B — Operator admin API (2–3 days)

- [x] CRUD `/v1/admin/apps`
- [x] Manual origin/bundle approve
- [x] Suspend / restore
- [x] Startup seed merge

**Done when:** apps manageable without DB access.

### Phase C — Domain verification (3–4 days)

- [x] DNS TXT + HTTPS well-known checkers
- [x] Challenge token rotation
- [x] `allowLocalhostOrigins` dev path

**Done when:** automated web origin verification e2e test passes.

### Phase D — Developer portal (5–7 days)

- [x] Developer auth (register/login JWT)
- [x] Self-service app CRUD
- [x] Email verification
- [x] Per-developer quotas

**Done when:** `registrationMode: self_service` end-to-end without operator.

### Phase E — Native + pairing scope (3–4 days)

- [x] Native bundle registration + manual approve
- [x] Optional client secret
- [x] `allowedAppIds` on pairing tokens
- [x] WS Origin validation

**Done when:** iOS/Android integration test with operator-managed bundle.

### Phase F — Documentation and OpenAPI (2 days)

- [x] OpenAPI paths for developer/admin app endpoints
- [x] Update agent docs, OPERATOR.md, CHANGELOG
...
- [x] `apps.enabled: false` — all v1.2 integration tests pass unchanged
- [x] `operator_managed` — operator creates app; client with matching origin syncs
- [x] Wrong origin → `403 APP_ORIGIN_NOT_ALLOWED`
- [x] Namespace created under app A inaccessible with app B credentials
- [x] `self_service` — developer verifies domain; app reaches `active` without operator
- [x] Localhost dev works with `allowLocalhostOrigins: true`
- [x] Pairing with `allowedAppIds` blocks wrong app
- [x] Dynamic CORS reflects verified origins only
- [x] No client secret in web SDK path
- [x] Migration assigns legacy namespaces via `legacyDefaultAppId`
