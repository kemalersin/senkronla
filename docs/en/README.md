...
| 16 | [16-APP-REGISTRY.md](./16-APP-REGISTRY.md) | **Application registry, namespace–app binding (spec v1.3 — shipped)** |
| 17 | [17-OPERATOR-LIMIT-OVERRIDES.md](./17-OPERATOR-LIMIT-OVERRIDES.md) | **Operator limit overrides — namespace / app / developer cascade (v1.3.2 — shipped)** |
...
This specification is application-agnostic. Each consumer app should maintain its own integration guide **outside** this folder (mapping local profile/workspace UUID to `namespaceId`, implementing `DocumentAdapter`, UI wiring). Do not add app-specific references to files in `en/` or `tr/`.
...
**v1.3.0 (shipped):** Application registry — operator-managed or self-service app registration; namespaces bound to apps — see [16-APP-REGISTRY.md](./16-APP-REGISTRY.md).

**v1.3.2 (shipped):** Operator limit overrides — namespace / app / developer cascade — see [17-OPERATOR-LIMIT-OVERRIDES.md](./17-OPERATOR-LIMIT-OVERRIDES.md).

---

| Field | Value |
|-------|-------|
| Spec version | 1.3.1 shipped (app registry, multi-document, WebSocket); 1.3.2 limit overrides |
