...
> **Bu dosyayı** implementasyon agent'ına birlikte `tr/` klasörünün tamamını (veya `../en/`) verin.
...
Tüm tasarım kararları `tr/` (Türkçe) veya `../en/` (İngilizce) içindedir. Çelişki durumunda öncelik:
...
Sapma gerekirse `../DEVIATIONS.md` oluştur ve gerekçelendir.
...
## Faz veya özellik yayına alındığında

Bir faz veya daha önce planlanmış bir yetenek tamamlandığında, **görevi kapatmadan önce dokümantasyonu senkronize et**:

1. [`.cursor/rules/feature-shipped-docs.mdc`](../../.cursor/rules/feature-shipped-docs.mdc) kuralını uygula — özellik spec'i, `11-IMPLEMENTATION-PLAN.md`, çapraz referans belgeleri (`07`, `08`, `10`, `12`, `OPERATOR.md`), `en/` ve `tr/` README indeksleri, OpenAPI, web/agent docs ve changelog'ları güncelle.
2. Davranış canlıysa *planlandı* ifadelerini *yayında* ile değiştir; faz checklist maddelerini `- [x]` işaretle.
3. Kapsamdaki eski gelecek zaman ifadelerini (`planned`, `planlandı`, `geliştirilecek`, …) ara ve düzelt.
...
1. `./README.md` (veya `../en/README.md`) oku
